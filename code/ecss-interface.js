...
findkey = function (array) {
  return function (key) {
    var index = _.findIndex(array.name, function (e) {
      return e == key
    })
...
getEnumerations = function () {
  ...
  http.open(
    "GET",
    "http://localhost:8090/api/mdb/fdirdemo/parameters/fdirdemo/Event_Definition_ID"
  )
  http.send()
  ...
  http2.addEventListener("load", function () {
    var json = JSON.parse(this.responseText)
    for (var value of _.find(json.type.member, { name: "Check_Status" }).type
      .enumValue) {
  ...
  http2.open(
    "GET",
    "http://localhost:8090/api/mdb/fdirdemo/parameters/fdirdemo/PMON_Monitoring_Definition"
  )
  http2.send()
}
getEnumerations()

getST12definitions = _.throttle(
  function () {
    pmons = {}
    var http = new XMLHttpRequest()
    http.open(
...
getST19requests = function () {
  for (key in events) {
    ...
    http.send(
      JSON.stringify({
        assignment: [{ name: "Event_Action_Definition_ID", value: key }],
      })
    )
  }
}
...
createPmonTable = _.throttle(
  function () {
    ...
    for (const [pmonId, pmon] of Object.entries(pmons)) {
      ...
      var tds = _.map(new Array(8), function (e) {
      ...
      if (pmon.validity) {
        var lis = _.map(new Array(2), function (e) {
        ...
        lis[1].childNodes[1].childNodes[0].appendChild(
          document.createTextNode(pmon.validity.mask)
        )
      ...
      if (pmon.check_type == "Limit_Check") {
        ...
        nodes[0].appendChild(
          document.createTextNode(
            pmon.check.low + " <= " + "x" + " <= " + pmon.check.high
          )
        )
        ...
        nodes[1].childNodes[0].appendChild(
          document.createTextNode(pmon.check.low_event)
        )
        ...
        nodes[2].childNodes[0].appendChild(
          document.createTextNode(pmon.check.high_event)
        )
        ...
        nodes[0] = document.createElement("p")
        nodes[0].appendChild(
          document.createTextNode("x" + " = " + pmon.check.value)
        )
        nodes[1] = document.createElement("span")
        nodes[1].classList.add("mdl-chip")
        ...
        nodes[1].childNodes[0].classList.add("mdl-chip__text")
        nodes[1].childNodes[0].appendChild(
          document.createTextNode(pmon.check.mask)
        )
        ...
        nodes[2].childNodes[0].classList.add("mdl-chip__text")
        nodes[2].childNodes[0].appendChild(
          document.createTextNode(pmon.check.event)
        )
...
createEventActionTable = _.throttle(
  function () {
    ...
    for (const [eventActionID, eventAction] of Object.entries(events)) {
      ...
      tds[1].childNodes[0].childNodes[0].appendChild(
        document.createTextNode(eventAction.event)
      )
      ...
      if (eventAction.request) {
        tds[3].appendChild(
          document.createTextNode(
            "TC[" +
              eventAction.request.service +
              "," +
              eventAction.request.message +
              "] "
          )
        )
        tds[3].appendChild(document.createElement("span"))
        tds[3].childNodes[1].classList.add("mdl-typography--font-light")
        tds[3].childNodes[1].appendChild(
          document.createTextNode(eventAction.request.content1 + " ")
        )
        tds[3].appendChild(document.createElement("span"))
        tds[3].childNodes[2].classList.add("mdl-typography--font-light")
        tds[3].childNodes[2].appendChild(
          document.createTextNode(eventAction.request.content2)
        )
      }
...
createTransitionTable = _.throttle(
  function () {
    ...
    for (const transition of _.reverse([...transitions])) {
      ...
      tds[4].appendChild(document.createElement("span"))
      tds[4].childNodes[0].appendChild(
        document.createTextNode(transition.previous)
      )
      colourStatus(tds[4].childNodes[0])
      tds[4].childNodes[1].appendChild(
        document.createTextNode("arrow_right_alt")
      )
      tds[4].childNodes[1].classList.add("material-icons")
      tds[4].childNodes[2].appendChild(
        document.createTextNode(transition.current)
      )
  ...
  if (json.type == "time") {
    ...
    if (eventActionRaw) {
      ...
      for (var definitionRaw of eventActionList) {
        ...
        var entry = {
          event:
            EventEnumeration[eventaction("Event_Definition_ID").uint32Value],
    ...
    if (eventActionRequestRaw) {
      var eventActionRequest = findkey(
        eventActionRequestRaw.engValue.aggregateValue
      )
    ...
    if (checkListRaw) {
      ...
      for (var checkRaw of checkList) {
        ...
        var transition = {
          ...
          previous:
            CheckStatusEnumeration[check("Previous_Check_Status").uint32Value],
          current:
            CheckStatusEnumeration[check("Current_Check_Status").uint32Value],
    ...
    if (monitoringRaw) {
      var checkRaw = _.find(json.data.values, function (e) {
        return e.numericId != 1
      })
      ...
      pmons[monitoring("PMON_ID").uint32Value] = {
        ...
        validity:
          Parameters[monitoring("Mask").uint32Value] == 0
            ? null
            : {
                parameter:
                  Parameters[monitoring("Validity_Parameter_ID").uint32Value],
                mask: "0x" + monitoring("Mask").uint32Value.toString(16),
                value: monitoring("Expected_Value").uint32Value,
              },
        monitoring_interval:
          monitoring("Monitoring_Interval").uint32Value + " ms",
