...
After defining the possible failure modes of the system, we are ready to analyse each entry to provide the correct software inputs to implement the required \ac{FDIR}. More specifically, for each failure, the following must be defined: \autocite[84]{SAVOIR-HB-003}
\begin{itemize}
...
The above information is listed in the \acf{HSIA} table, which links each failure (identified in the \ac{FMEA}) to the corresponding low-level software parameters (\Cref{tab:hsia}).
...
\begin{table}[h]
	\centering
	\caption{List of \texttt{ST[20]} parameters\\R: Read-only\\RW: Read-write}
...
\begin{table*}[h]
	\centering
	\caption[][5pt]{List of \texttt{ST[12]} monitoring definitions. \(t\) and \(\Delta\) are used as placeholders for parameter values.}
...
\begin{table}[h]
	\centering
	\caption{\texttt{ST[19]} event-action definitions}
