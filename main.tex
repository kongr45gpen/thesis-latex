...
\eqref{eq:dual_redun} applies to both cold and hot redundancy, as we assume that the system can recover from permanent failures in both cases, moving control to the unaffected component.\footnote{Some cold-redundant systems may offer even higher reliability, if the unused component is not powered and is unaffected by the relevant failure cause.}

The reliability of the \emph{triple modular redundancy} system is \autocite[31]{birolini_reliability_engineering_2004}:
\begin{align}
...
It is worth noting that the \acs{TMR} system considers a simple voter, and in the case of \emph{permanent failure of both vulnerable components, the system is considered unable to recover.}
...
\begin{figure}
	...
	\caption[Resilience of a compound system to permanent failures (assuming a vulnerable voter)]{Resilience of a compound system to permanent failures (assuming a vulnerable voter --- higher \(R\) is better)}
...
\marginnote{Alternatively, we can ignore the negligible 2\textsuperscript{nd} term of \eqref{eq:lambda}, and have \( P_2 = 2\lambda e^{-\lambda} \). Since \(e^x \approx 1\) very close to \(x=0\), \eqref{eq:lambbert_eqn} becomes:
				\[
				\lambda_2 = 2\lambda
				\]
...
\begin{table*}[h]
	\caption[][1ex]{Summary of reliable architecture analysis}
	...
	\begin{tabular}{@{}rllX@{}}
		...
		Dual redundancy, cold & \color{MaterialGreen600} Very good & \color{MaterialGrey800} Normal & \color{MaterialOrange900} Medium\\
		Dual redundancy, hot & \color{MaterialGreen600} Very good & \color{MaterialOrange900} Worse & \color{MaterialOrange900} Medium\\
		\acl{TMR} & \color{MaterialTeal900} Better {\small(if \(R > 0.5\))} & \color{MaterialGreenA700} Excellent & \color{MaterialRed800} High
	\end{tabular}
\end{table*}
