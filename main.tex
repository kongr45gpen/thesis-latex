...
\begin{compactitem}
	...
	\begin{marginfigure}
		...
		\caption[Example mission image output]{Example mission image output (\cite{DDJF_PL})}
...
\textbf{Hot redundancy:} Two or more simultaneously active parts operate in parallel. \cite{SAVOIR-HB-003}
}

The two sensors are wired in a \textbf{hot-redundant}\footnote{Meaning they are both operating and generating measurements at the same time \draft{(maybe add a full definition here)}} configuration, due to their extremely low operating power. The two sensors are connected to different \ac{I2C} buses, so that the failure of one bus will not affect the operation of the other sensor.
...
\begin{figure}[h]
	\missingfigure[figwidth=6cm]{electrical schematic}
...
\marginnote{Double failures are not investigated in the scope of this document, as they are mostly considered only for launchers and manned missions \cite{SAVOIR-HB-003}.}
...
\marginnote{For this demonstration, we will not investigate failures of the microcontroller itself and its internals. Refer to \cite{retselis_acubesat_fmea_2020} for AcubeSAT's \acs{FMEA} and \acs{HSIA} on the microcontrollers.}
\begin{compactenum}
...
The power of \acs{SAVOIR}'s \ac{FDIR} approach lies in the fact that the entire \ac{FDIR} concept can be safely modified via \ac{TC} without requiring any modifications or reuploads of the spacecraft's software.
