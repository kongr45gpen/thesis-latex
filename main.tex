...
\hypersetup{
	pdftitle={Design of FDIR in the AcubeSAT nanosatellite},
	pdfsubject={Design of Fault Detection, Isolation and Recovery in the AcubeSAT Nanosatellite},
...
\par The views expressed herein by the author can \smallcaps{in no way be taken to reflect} the official opinion, or endorsement, of the European Space Agency.
...
\marginnote[1cm]{%
	...
	\textbf{Event}: Occurrence of a set of conditions that can arise during the mission, such as:
	\begin{compactitem}
...
The \href{https://www.microchip.com/en-us/development-tools-tools-and-software/embedded-software-center/mplab-harmony-v3}{MPLAB Harmony}\footnote[]{\url{https://www.microchip.com/en-us/development-tools-tools-and-software/embedded-software-center/mplab-harmony-v3}} \ac{HAL} and configurator are used to interface with all the \acs{MCU}'s peripherals. \href{https://www.jetbrains.com/clion/}{CLion}\footnote[][3ex]{\url{https://www.jetbrains.com/clion/}} has been selected as the \acs{IDE}, along with the \foothref{https://cmake.org/}{CMake} build system.
...
\begin{table*}[h]
	...
	\begin{tabular}{@{}L{1cm}L{2.5cm}L{2.8cm}L{2cm}L{2cm}L{2cm}ll@{}}
		...
		\midrule
		10 & Temperature 1 & Temp. 1 Status & \texttt{NOMINAL} & \SI{500}{\milli\second} & 12 & Range & \( t < \SI{50}{\celsius} \) \\
...
During nominal operation, no failures are simulated. The system is connected to power and outputs \acs{ECSS} telemetry (\Cref{fig:yamcshousekeeping}), which includes parameter values parsed by (\Cref{fig:yamcsparameter}). Additionally, a log file is kept for diagnostic purposes (\Cref{fig:lognominal}). The system is compliant with its requirements (\Cref{sec:tsvcd}) and can transmit the temperature to the \acl{GS} as expected.
...
It is of particular interest to examine the state of the \acs{FDIR} system during microcontroller boot. The first \acs{ECSS} data we receive are shown in \Cref{fig:pus_boot}. The image shows the \textbf{transitions} of the \emph{\texttt{ST[12]} on-board monitoring} service, as shown in the interface we developed (\Cref{sec:pusinterface}). In practice, this view shows the \textbf{result} of each check, and if the value is within bounds, out of bounds, or if the check is disabled.
...
At startup, an \emph{\texttt{ST[05]} event reporting} event is also generated, which announces the microcontroller startup (\Cref{fig:yamcsmcustart}).
...
\clearpage
The following paragraphs will describe the simulation procedure and results for each failure of \Cref{tab:testfailures}.
...
\begin{table*}[h]
	...
	\begin{tabularx}{\linewidth}{@{}S[table-format=4.0]c@{\hskip 3em}rcl@{\hskip 3em}X@{}}
		...
		0 & 6 & Temp. \(\Delta\) & = & 25.6875 & \ok \ar \invalid  \\
		0 & 7 & Temp. 1 & = & 0 & \ok \ar \invalid \\
		0 & 9 & Temp. \(\Delta\) & = & 25.6875 & \ok \ar \invalid \\
...
After updating the parameter values, the \emph{\texttt{ST[12]} on-board monitoring} service and its monitoring definitions (\Cref{fig:pusf010tran,tab:f010tran}) are next. The following steps are taken in order:
\begin{enumerate}
	\item \emph{Immediately} after the sensor is disconnected, definitions 4, 6, 7, 9, and 10 (\Cref{tab:demo_monitoring}) acquire an \invalid{} state. This happens after deactivation of the \textbf{check validity condition} for these definitions. If sensor 1 is disabled,\footnote[][8ex]{either due to a problem or a command} its values should not be used for any on-board functionality.
	
	Therefore, all definitions associated with sensor 1 are disabled and cannot trigger any reaction. This is done immediately.
	\item After two measurements and \( 2 \cdot 500 = \SI{1000}{\milli\second} \) of time, monitoring definition 0 confirms that the parameter \emph{Temperature Status 1} has an unexpected value. As a result, the monitoring definition is switched to the \unexpected{} state, therefore triggering the entire fault reporting and recovery process. The sensor failure has now been detected, and the \acs{FDIR} system is now in action.
...
\begin{table*}[h]
	\centering
	\caption[]{Monitoring definition transitions table (\texttt{F-020} failure)}
	\label{tab:f020tr}
	\begin{tabularx}{\linewidth}{@{}S[table-format=4.0]c@{\hskip 3em}rcl@{\hskip 3em}X@{}}
		\toprule
...
\begin{table*}[h]
	...
	\begin{tabularx}{\linewidth}{@{}S[table-format=4.0]c@{\hskip 3em}rcl@{\hskip 3em}X@{}}
		\toprule
		\multicolumn{1}{l}{Time (s)} & \acs{PMON} & \multicolumn{3}{c@{\hskip 3em}}{Parameter} & Transition \\ \midrule
		1 & 4 & Temp. 1 & = & 109.4375 & \ok \ar \hilim \\[1ex]
		\multicolumn{6}{c}{\textcolor{MaterialGrey500}{Failure recovery}}\\[1ex]
		2.5 & 4 & Temp. 1 & = & 29.5625 & \hilim \ar \ok \\
		\bottomrule
	\end{tabularx}
	\vspace{10pt}
\end{table*}
...
This fault is similar to \texttt{F-040}, but with sensor repair being impossible. Button presses were once again used to simulate the fault in software.
...
\begin{table*}[h]
	...
	\begin{tabularx}{\linewidth}{@{}S[table-format=4.0]c@{\hskip 3em}rcl@{\hskip 3em}X@{}}
		\toprule
		\multicolumn{1}{l}{Time (s)} & \acs{PMON} & \multicolumn{3}{c@{\hskip 3em}}{Parameter} & Transition \\ \midrule
		1 & 4 & Temp. 1 & = & 109.4375 & \ok \ar \hilim \\
		2.5 & 7 & Temp. 1 & = & 109.4375 & \ok \ar \hilim \\[1ex]
		\multicolumn{6}{c}{\textcolor{MaterialGrey500}{Failure isolation}}\\[1ex]
		2.5 & 9 & Temp. \( \Delta \) & = & \textminus79.9375 & \ok \ar \invalid \\
		2.5 & 10 & Temp. 1 & = & 109.4375 & \ok \ar \invalid \\
		3 & 4 & Temp. 1 & = & 109.4375 & \hilim \ar \invalid \\
		3 & 6 & Temp. \( \Delta \) & = & \textminus79.9375 & \ok \ar \invalid \\
		3 & 7 & Temp. 1 & = & 109.4375 & \hilim \ar \invalid \\
		\bottomrule
	\end{tabularx}
	\vspace{10pt}
\end{table*}
...
The \texttt{F-040} and \texttt{F-050} failures involve an auxiliary detection method: The \emph{large difference between 2 sensor measurement values}. Given the small threshold defined in \Cref{tab:hsia} (\(\lvert \Delta \rvert \leq \SI{5}{\celsius}\)), this test is performed in hardware, by heating one of the two sensors.
...
\begin{table*}[h]
	...
	\begin{tabularx}{\linewidth}{@{}S[table-format=4.0]c@{\hskip 3em}rcl@{\hskip 3em}X@{}}
		\toprule
		\multicolumn{1}{l}{Time (s)} & \acs{PMON} & \multicolumn{3}{c@{\hskip 3em}}{Parameter} & Transition \\ \midrule
		3 & 6 & Temp. \( \Delta \) & = & \textminus6.1875 & \ok \ar \lolim \\[1ex]
		\multicolumn{6}{c}{\textcolor{MaterialGrey500}{Failure recovery}}\\[1ex]
		10.5 & 6 & Temp. \( \Delta \) & = & \textminus4.8125 & \lolim \ar \ok \\
		\bottomrule
	\end{tabularx}
	\vspace{10pt}
\end{table*}
...
\begin{table*}[h]
	...
	\begin{tabularx}{\linewidth}{@{}S[table-format=4.0]c@{\hskip 3em}rcl@{\hskip 3em}X@{}}
		\toprule
		\multicolumn{1}{l}{Time (s)} & \acs{PMON} & \multicolumn{3}{c@{\hskip 3em}}{Parameter} & Transition \\ \midrule
		3 & 6 & Temp. \( \Delta \) & = & \textminus5.8125 & \ok \ar \lolim \\
		5.5 & 9 & Temp. \( \Delta \) & = & \textminus6 & \ok \ar \lolim \\[1ex]
		\multicolumn{6}{c}{\textcolor{MaterialGrey500}{Failure isolation}}\\[1ex]
		5.5 & 10 & Temp. 1 & = & 38.3125 & \ok \ar \invalid \\
		5.5 & 11 & Temp. 2 & = & 32.3125 & \ok \ar \invalid \\
		6 & 4 & Temp. 1 & = & 38.3125 & \ok \ar \invalid \\
		6 & 5 & Temp. 2 & = & \textminus32.3125 & \ok \ar \invalid \\
		6 & 6 & Temp. \( \Delta \) & = & -6 & \ok \ar \invalid \\
		6 & 7 & Temp. 1 & = &38.3125 & \ok \ar \invalid \\
		6 & 8 & Temp. 2 & = & 32.3125 & \ok \ar \invalid \\
		6 & 9 & Temp. \( \Delta \) & = & \textminus6 & \ok \ar \invalid \\
		\bottomrule
	\end{tabularx}
	\vspace{10pt}
\end{table*}
...
\begin{table*}[h]
	...
	\begin{tabularx}{\linewidth}{@{}S[table-format=4.0]c@{\hskip 3em}rcl@{\hskip 3em}X@{}}
		\toprule
		\multicolumn{1}{l}{Time (s)} & \acs{PMON} & \multicolumn{3}{c@{\hskip 3em}}{Parameter} & Transition \\ \midrule
		6 & 10 & Temp. 1 & = & 92.288 & \ok \ar \hilim \\
...
In this example, only the monitoring definition linked with the \SI{50}{\celsius} value is activated. Once the system spends some time in this temperature, an \acs{MCU} restart takes place immediately. This restart would continue to occur repeatedly, if the temperature remained out of bounds.
...
\clearpage
\subsection{Real-time modification of \acs{FDIR}}
...
\begin{table*}[h]
	\centering
	\caption{\acs{FDIR} reconfigurability requirements}
...
With the above set of telemetry, downlinked either periodically or after \acs{TC}, operators are able to examine the state of the system and its \acs{FDIR} in detail. All the results of \Cref{sec:simul} were extracted using the above telemetry structures.
...
\begin{table*}[h]
	...
	\begin{tabularx}{\linewidth}{@{}S[table-format=4.0]c@{\hskip 3em}rcl@{\hskip 3em}X@{}}
		\toprule
		\multicolumn{1}{l}{Time (s)} & \acs{PMON} & \multicolumn{3}{c@{\hskip 3em}}{Parameter} & Transition \\ \midrule
		1 & 4 & Temp. 1 & = & 110.625 & \ok \ar \hilim \\
		2.5 & 7 & Temp. 1 & = & 110.6875 & \ok \ar \hilim \\
		3 & 6 & Temp. \( \Delta \) & = & \textminus79.9375 & \ok \ar \lolim \\
		5.5 & 9 & Temp. \( \Delta \) & = & \textminus79.875 & \ok \ar \lolim \\
		6 & 10 & Temp. 1 & = & 110.625 & \ok \ar \hilim \\
		\bottomrule
	\end{tabularx}
	\vspace{.5em}
\end{table*}
